...
class ClassicalMemory(ReplayMemory):
    ...
    def __init__(self, length, batch_size): # max_length, batch_size
    ...
    def update(self, fake, t_logit):
        ...
        if self.fakes is None:  # Initialize circular buffer if it hasn't already been.
            self.fakes = torch.zeros((self.max_length, fake.shape[1], fake.shape[2], fake.shape[3])) # 存入图像
            self.logits = torch.zeros((self.max_length, t_logit.shape[1])) # 大维度
        tail = self.head + fake.shape[0] # 保存
        ...
        self.head = tail % self.max_length #  set head
    ...
    def sample(self):
        ...
        idx = perm(self.size, self.batch_size).cpu()
        return self.fakes[idx].cuda(), self.logits[idx].cuda()
...
def init_replay_memory(args):
    ...
    if args.replay == "Classic":
        return ClassicalMemory(args.replay_size, args.batch_size)
    raise ValueError(f"Unknown replay parameter {args.replay}")

def perm(pop_size, num_samples):
    ...
    return torch.randperm(pop_size)[:num_samples]
